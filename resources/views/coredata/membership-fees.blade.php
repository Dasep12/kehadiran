        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div id="membership-fees-master"></div>
                </div>
            </div>
        </div>
    </div>
...
@push('scripts')
<script>
    var table = new Tabulator("#membership-fees-master", {
        ajaxURL: "{{ route('sallaryTax.getMembershipFeesData') }}", // endpoint Laravel
        ajaxConfig: "GET",
        ...
        columns: [{
                ...
                cellClick: function(e, cell) {
                    ...
                    new Tabulator(tableEl, {
                        ajaxURL: "{{ route('sallaryTax.getMembershipFeesDataRate') }}",
                        layout: "fitData",
                        ajaxParams: {
                        ...
                        columns: [{
                                ...
                                formatter: function(cell) {
                                    ...
                                    let cleanVal = parseFloat(val); // Bersihkan 0000
                                    let data = cell.getData();

                                    if (data.calculation_type === 'PERCENT') {
                                        return cleanVal + " %"; // Hasil: 3.7 %
                                    } else {
                                        // Format Rupiah untuk FIXED
                                        return "Rp " + cleanVal.toLocaleString("id-ID"); // Hasil: Rp 3.700
                                    }
                                }
                            }, {
                                title: "Start Date",
    ...
    function actionFormatter(cell) {
        return `<button type="button" onclick="Crud('update', '${cell.getRow().getData().membership_id}')" class="btn btn-sm btn-outline-primary me-1">
        ...
        <button type="button" onclick="Crud('delete', '${cell.getRow().getData().membership_id}')" class="btn btn-sm btn-outline-danger">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-1">
    ...
    function reloadTable() {
        ...
        table.setData("{{ route('sallaryTax.getMembershipFeesData') }}", {
            search: search
        });
    }
</script>
...
@include('coredata.partials.crud-membership-fees');
@endsection