        ...
        <div class="offcanvas-body">
            <div id="content-crud">
                <div class="mb-3 row">
                    ...
                    <div class="col">
                        <input type="text" name="code_attendance" id="code_attendance" class="form-control" aria-describedby="emailHelp" placeholder="Enter ID">
                    </div>
                </div>
                <div class="mb-3 row">
                    <label class="col-3 col-form-label required">Attendance Name</label>
                    <div class="col">
                        <input type="text" name="name_attendance" id="name_attendance" class="form-control" aria-describedby="educationHelp" placeholder="Enter name">
                    </div>
                </div>

                <div class="mb-3 row">
                    <label class="col-3 col-form-label required">Status Name</label>
                    <div class="col">
                        <select class="form-control" name="status_attendance" id="status_attendance">
                ...
                <div class="mb-3 row">
                    ...
                    <div class="col">
                        <input type="text" name="remarks" id="remarks" class="form-control" aria-describedby="educationHelp" placeholder="Enter remarks">
                    </div>
                </div>
...
<script>
    function Crud(action, id) {
        ...
        $('#code_attendance').attr('readonly', false); // ID biasanya selalu readonly
        ...
        if (id !== '*') {
            ...
            $('#code_attendance').val(data.code_attendance);
            $('#name_attendance').val(data.name_attendance);
            $('#status_attendance').val(data.status_attendance);
            $('#remarks').val(data.remarks);
        }

        switch (action) {
            case 'create':
                $('#offcanvasEndLabel').text('Create Attendance Types');
                break;

            case 'update':
                $('#code_attendance').attr('readonly', true); // ID tidak bisa diubah saat update
                $('#offcanvasEndLabel').text('Edit Attendance Types');
                break;

            case 'delete':
                $('#offcanvasEndLabel').text('Delete Attendance Types');
    ...
    $('#form-crud').on('submit', function(e) {
        ...
        let url = '{{ route("worktime.CrudAttendaceTypes") }}';
        let method = 'POST';

        let formData = {
            code_attendance: $('#code_attendance').val(),
            name_attendance: $('#name_attendance').val(),
            status_attendance: $('#status_attendance').val(),
            remarks: $('#remarks').val(),
            action: action,
            _token: '{{ csrf_token() }}'
        };
