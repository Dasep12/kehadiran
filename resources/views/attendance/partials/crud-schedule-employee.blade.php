        ...
        <div class="offcanvas-body">
            <div id="content-crud">
                ...
                <div class="mb-3 row">
                    ...
                    <div class="col">
                        <input type="text" name="start_date" id="start_date" class="form-control date_picker" aria-describedby="educationHelp" placeholder="Enter Start Date">
                    </div>
                </div>
                ...
                <div class="mb-3 row">
                    ...
                    <div class="col">
                        <input type="text" name="end_date" id="end_date" class="form-control date_picker" aria-describedby="educationHelp" placeholder="Enter End Date">
                    </div>
                </div>
...
<script>
    function Crud(action, id) {
        ...
        switch (action) {
            case 'create':
                $('#offcanvasEndLabel').text('Create Employee Shift');
                break;

            case 'update':
                $('#id').attr('readonly', true); // ID tidak bisa diubah saat update
                $('#start_date').attr('disabled', true); // ID tidak bisa diubah saat update
                $('#employee_id').attr('disabled', true); // ID tidak bisa diubah saat update
                $('#shift_group_id').attr('disabled', true); // ID tidak bisa diubah saat update
                $('#offcanvasEndLabel').text('Edit Employee Shift');
                break;
            ...
            case 'delete':
                ...
                $('#form-crud input').attr('disabled', true);
    ...
    function loadGroupShift() {
        $.ajax({
            ...
            success: function(response) {
                let options = '<option value="all">Select All Group Shift</option>';
    ...
    $('#form-crud').on('submit', function(e) {
        ...
        let url = '{{ route("attendance.GenerateSchedule") }}';
        let method = 'POST';
