...
@push('scripts')
<script>
    var table = new Tabulator("#tax-ptkp-master", {
        ajaxURL: "{{ route('sallaryTax.getTaxPtkpData') }}", // endpoint Laravel
        ajaxConfig: "GET",
        // 🔥 layout fix (penting)
        // layout: "fitData",
        layout: "fitColumns",
        ...
        index: "ptkp_code", // Set index untuk operasi update/delete
        columns: [{
                title: "ptkp_code",
                field: "ptkp_code",
                width: 150
    ...
    function actionFormatter(cell) {
        return `<button type="button" onclick="Crud('update', '${cell.getRow().getData().ptkp_code}')" class="btn btn-sm btn-outline-primary me-1">
        ...
        <button type="button" onclick="Crud('delete', '${cell.getRow().getData().ptkp_code}')" class="btn btn-sm btn-outline-danger">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-1">
    ...
    function reloadTable() {
        ...
        table.setData("{{ route('sallaryTax.getTaxPtkpData') }}", {
            search: search
        });
    }
</script>
...
@include('sallary-tax.partials.crud-tax-ptkp');
@endsection